...
all_by_country <- inner_join(inner_join(c_by_country, d_by_country, by = "Country.Region"), r_by_country, by = "Country.Region") %>% mutate(Country.Region = as.character(Country.Region))
...
all$Density[all$Country.Region == "Cape Verde"] = wppd$Density[wppd$Country.Region == "Cabo Verde"]
all$Population[all$Country.Region == "Cape Verde"] = wppd$Population[wppd$Country.Region == "Cabo Verde"]
# There are two Congo entries in covid-19 data, one congo entry from the world population data
all$Density[str_detect(all$Country.Region, "Congo")] = wppd$Density[wppd$Country.Region == "Congo"]
all$Population[str_detect(all$Country.Region, "Congo")] = wppd$Population[wppd$Country.Region == "Congo"]/2
# There is no entries for Kosovo in world population data. Cruise ship is not a real country. 
all$Density[str_detect(all$Country.Region, "Cruise Ship")] = 100
...
all <- all %>% 
  mutate("D/C %" = (Deaths/Confirmed)*100, "R/C %" = (Recovered/Confirmed)*100) %>% 
  mutate("C/Population" = (Confirmed/Population)*10000, "D/Population" = (Deaths/Population)*10000, "R/Popluation" = (Recovered/Population)*10000) %>%
  mutate("D/C % by Density" = `D/C %`/Density, "R/C % by Density" = `R/C %`/Density) %>% 
  arrange(desc(Confirmed)) %>% slice(1:50)

# knitr::kable(all, 
#              caption = "World Covid-19 Summary 03/21/2020\n C/Population, D/Population, R/Population are per 10,000 people")
```
...
1. If just comparing number of Confirmed/Deaths/Recovered cases, China has the highest confirmed case. Italy has the highest 
...
2. Adding the consideration of population density, Iran has the highest Deaths/Confirmed cases by density. 
